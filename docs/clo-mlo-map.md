...
2. **CLO-5 assessment — partially resolved.** A common rubric is wanted rather than an instructor-by-instructor standard. **Decided:** a lightweight rubric, four categories summing to 20 points. **Still open:** what the four categories are and how points split among them. [rubric.md](rubric.md) today is a 12-category, 24-point rubric for evaluating *lesson and assignment design* — a different artifact from a rubric for scoring *student test evidence* — so this is a new document, not an edit to the existing one, and it should not be authored until the four categories are named.
