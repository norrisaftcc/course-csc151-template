...
1. ~~**CLO-3 depth.**~~ **Resolved.** "Object-oriented principles" means encapsulation — fields, methods, and a constructor. Inheritance and polymorphism are out of scope for CSC 151 and belong to the follow-on course, CSC 251. Submodule 8.1 may still show inheritance and interfaces as an optional dual-enrollment extension (see [course-map.md](course-map.md) and [pacing-guide.md](pacing-guide.md)), but no CLO-3 task requires them.
2. ~~**CLO-5 assessment.**~~ **Resolved.** A common rubric replaces an instructor-by-instructor standard: a lightweight rubric, four categories summing to 20 points. The four categories are the course's own three-tier evidence model plus the explanation step that already follows it — **normal case, boundary case, failure case, and explanation, five points each.** [rubric.md](rubric.md) stays a 12-category, 24-point rubric for evaluating *lesson and assignment design*, a different artifact from a rubric for scoring *student test evidence*, so the decision lands in a new document: [test-evidence-rubric.md](test-evidence-rubric.md).
3. ~~**Module 0 credit.**~~ **Resolved.** Module 0 is ungraded prerequisite time — the period in which new students get logged in and get the tools working, not a first graded milestone.
4. ~~**CLO-4 without a graphical display.**~~ **Resolved.** CLO-4 is fulfilled by processing a user event as a function call. A console menu selection and a Swing button press are both accepted evidence, and the console path is not the lesser one. Submodule 8.2 therefore carries no local-machine requirement, and every graded task runs in a Codespace. Swing examples are kept in the course as alternatives or worked examples, not as a required path.
5. **AI teammate disclosure:** Confirm that the disclosure standard in the [AI Teammate Policy](ai-teammate-policy.md) matches the institution's academic integrity policy. **Status: pending human review.**
