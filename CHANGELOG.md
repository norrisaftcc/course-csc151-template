...
- **Reconciled the curriculum content with the instructor's answers to the open-questions list.** CLO-3, Module 0 credit, and CLO-4 are now marked Resolved with strikethrough, matching the document's own stated convention for closed items. CLO-5's rubric question was left partially resolved at this point — the point total was set, but the four categories were not yet named — and a follow-up entry below (`### Added`, `docs/test-evidence-rubric.md`) records the four categories being named and the question closed out.
...
- `modules/m8-events-and-capstone/lesson-3-swing-track-addendum.md` — Module 8a, an optional local Swing track that wires existing handlers to buttons without changing the console path or grading access.
- The Canvas compositor now uses a black-and-gold default palette with grayscale surfaces, and the former Swing-track and palette decisions are resolved.
- `docs/status-2026-07-31.html` — a completion status dashboard for the template, built for a face-to-face demo with an instructor who has not seen the repository. One self-contained HTML file with no network requests; it opens offline from a local clone in any browser. The numbers come from a live file scan and a real `verify.sh all` run at the generating commit, never from hand-written figures: 10 of 10 spine units with a README, 13 of 13 lessons, 21 of 21 examples verified against their recordings, 42 of 42 verifier checks passed. The page states plainly that the four JUnit test files exist but were not executed in the generating environment, and gives the two commands to run them. Status is always an icon plus a word, never color alone; the page follows the viewer's light or dark preference; course terms carry a gloss at first use. It is a snapshot — regenerate it rather than editing the numbers.
- `docs/test-evidence-rubric.md` — the CLO-5 assessment rubric that open question 2 in `docs/clo-mlo-map.md` had left partially resolved, now fully resolved. Four categories, five points each, twenty points total: normal case, boundary case, failure case, and explanation. Each category scores whichever submission form a module actually grades — a manually recorded Test evidence table (Tier, Expected, Actual, Match?), used by every module except one, or a submitted JUnit test class named with the `method_expectedResult_forCondition` convention, used only by Module 5 Lesson 5.2 — grounded in real examples from `modules/m6-arrays-collections/tests/ArrayStatsTest.java` and `modules/m7-classes-objects/tests/BookTest.java` rather than a generic rubric. `docs/rubric.md` is unaffected; it scores lesson and assignment design, a different artifact from student test evidence.
