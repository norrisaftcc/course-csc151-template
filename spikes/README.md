...
- **The result retracts if you edit the huddle after the snap.** Revising a prediction while
  the answer is on screen is the exact habit the page argues against, so the page does not
  permit it either. Change either field and the output and its verdict disappear until you
  snap again.
...
- It is not accessible-audited beyond the basics. It carries a plain-text description, a
  `<title>` and `<desc>` on the diagram, labelled form fields, no meaning carried by color
  alone, and it honours `prefers-reduced-motion`. It has not been tested with a screen reader.
...
- The retraction was checked both ways: editing the prediction and editing the success check
  each hide the result and unlight the Snap chips, and snapping again works afterwards.
- Both runs were repeated with `prefers-reduced-motion: reduce` set.
